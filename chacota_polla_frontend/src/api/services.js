...
export const api = {
  ...
  jugadores: (equipo) => http.get(`/api/jugadores${equipo ? `?equipo=${equipo}` : ""}`),
  partidos: () => http.get("/api/partidos"),

  grupos: () => http.get("/api/grupos"),
  misGrupos: (idUsuario) => http.get(`/api/grupos/mis-grupos?usuario=${idUsuario}`),
  crearGrupo: (payload) => http.post("/api/grupos", payload),
  unirseGrupo: (payload) => http.post("/api/grupos/unirse", payload),
  actualizarCupo: (idGrupo, payload) => http.put(`/api/grupos/${idGrupo}/cupo`, payload),
