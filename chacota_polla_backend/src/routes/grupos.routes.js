...
import {
  rankingGrupo,
  rankingGeneralGrupos,
  rankingGeneralEmpresa
...
export async function gruposRoutes(request, env, path, method, url) {
  try {
    ...
    if (method === "GET" && path === "/api/grupos/ranking-general") {
      return ok(
        await rankingGeneralGrupos(env.DB),
        "Ranking general de grupos obtenido"
      );
    }

    if (method === "GET" && path === "/api/grupos/ranking-empresa") {
      return ok(
        await rankingGeneralEmpresa(env.DB),
        "Ranking general de empresa obtenido"
      );
    }
    ...
    if (method === "POST" && path === "/api/grupos") {
      return ok(
        await crearGrupo(env.DB, await readBody(request)),
        "Grupo creado"
      );
    }
    ...
    if (method === "POST" && path === "/api/grupos/unirse") {
      return ok(
        await unirsePorCodigo(env.DB, await readBody(request)),
        "Te uniste al grupo"
      );
    }
    ...
    if (method === "GET" && rankingMatch) {
      return ok(
        await rankingGrupo(env.DB, Number(rankingMatch[1])),
        "Ranking del grupo obtenido"
      );
    }
